...
def scrape_data(url):
    ...
    # Scrape a single URL  --> here is the part to increase limit of extraced raws from 10 to 1000
    # scraped_data = app.scrape_url(url,{'pageOptions':{'onlyMainContent': True}})
    scraped_data = app.scrape_url(url, {'pageOptions': {'onlyMainContent': True, 'mainContentLimit': 100000}})
...
def format_data(data, fields=None):
    ...
    if fields is None:
        fields = ["Name", "Price", "1h %", "24h %", "7d %", "Market Cap", "Volume(24h)", "Circulating Supply"]
